...
const INFO = 
{
  /** name    */ LIBRARY : "Dw.EasyCam",
  /** version */ VERSION : "1.0.9",
  /** author  */ AUTHOR  : "Thomas Diewald",
  /** source  */ SOURCE  : "https://github.com/diwi/dw-webgl-sketchbook/dwgl/",
...
class EasyCam {
  ...
  constructor(parent, args) {
    ...
    if(args.viewport === undefined) args.viewport  = [0, 0, canvas.width, canvas.height];
    ...
    this.mouse = {
      ...
      updateInput : function(x,y,z){
        ...
        mouse.dist[2] = -(mouse.curr[2] - mouse.prev[2]);
      },
  ...
  /** @return {canvas} the currently used canvas */
  getCanvas(){
    return this.canvas;
  }
  ...
  /** attaches input-listeners (mouse, touch, key) to the used canvas */
  attachMouseListeners(canvas){
    var cam = this.cam;
    var mouse = cam.mouse;
    
    canvas = canvas || cam.canvas;
    if(canvas){
      
      var op = { passive:false };

      cam.attachListener(canvas  , 'mousedown' , mouse.mousedown , op);
      cam.attachListener(document, 'mouseup'   , mouse.mouseup   , op);
      cam.attachListener(document, 'mousemove' , mouse.mousedrag , op);
      cam.attachListener(canvas  , 'dblclick'  , mouse.dblclick  , op);
      cam.attachListener(canvas  , 'wheel'     , mouse.wheel     , op);
      cam.attachListener(canvas  , 'touchstart', mouse.touchstart, op);
      cam.attachListener(canvas  , 'touchend'  , mouse.touchend  , op);
      cam.attachListener(canvas  , 'touchmove' , mouse.touchmove , op);
  ...
  removeMouseListeners(){
    ...
    cam.detachListener(mouse.mousedrag );
  ...
  apply(m4_camera) { 
    ...
    if(m4_camera){
      ...
      mat4.lookAt(m4_camera, this.camEYE, this.camLAT, this.camRUP);
    }
    return this;
  }
  ...
  beginHUD(def) {
    ...
    var gl = def.gl;
    var m4_modelview  = def.m4_modelview;
    var m4_projection = def.m4_projection;
    ...
    if(!this.pushed){
      ...
      this.pushed.m4_projection = mat4.create();
    }
    mat4.copy(this.pushed.m4_modelview , m4_modelview );
    mat4.copy(this.pushed.m4_projection, m4_projection);
    // 3) set new modelview (identity)
    mat4.identity(m4_modelview);
    // 4) set new projection (ortho)
    mat4.ortho(m4_projection, 0, w, -h, 0, -d, +d);
    // renderer.ortho();
  ...
  endHUD(def) {
    ...
    mat4.copy(m4_modelview , this.pushed.m4_modelview );
    mat4.copy(m4_projection, this.pushed.m4_projection);
    // 1) enable DEPTH_TEST
